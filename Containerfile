...
RUN dnf update -y && \
    dnf install -y git cmake gcc-c++ make curl-devel json-c-devel libusbx-devel opencv-devel asio-devel nlohmann-json-devel mbedtls-devel libX11-devel libX11-devel glib2-devel pipewire-devel && \
    dnf clean all
